...
function updateTable(productData) {
    ...
    var existingRow = tableBody.find('tr[data-product-id="' + productData.product_id + '"]');
    ...
    if (existingRow.length > 0) {
        ...
        existingRow.find('.quantity_placeholder').html(`<input type="number" name="quantity" class="quantity_input" value="${productData.quantity || ''}" onchange="updatePrice(this)">`);
        existingRow.find('.volts_placeholder').html(`<input type="text" name="volts[]" class="volts_input" value="${productData.volts || ''}" onchange="handleVoltsChange(this)">`);
        existingRow.find('.rental_hours_placeholder').html(`<input type="text" name="rental_hours[]" class="rental_hours_input" value="${productData.rental_hours || ''}">`);
        existingRow.find('.unit_price_placeholder').html(`<input type="text" name="unit_price" class="unit_price_input" value="${productData.unit_price || ''}" onchange="updatePrice(this)">`);
        existingRow.find('.discount_placeholder').html(`<input type="text" name="discount[]" class="discount_input" value="${productData.discount || '0%'}" onfocus="formatDiscountOnFocus(this)" onblur="formatDiscountOnBlur(this)">`);
        existingRow.find('.price_placeholder').text(calculatePrice(existingRow).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }));
        existingRow.find('.extra_hours_placeholder').html(`<input type="text" name="extra_hours" class="extra_hours_input" value="${productData.extra_hours || ''}">`);

        // adicionando uma nova linha
    } else {
        var newRow = $('<tr data-product-id="' + productData.product_id + '">');
...
function updateTableService(serviceData) {
    ...
    if (existingRow.length > 0) {
        ...
        existingRow.find('discount_service_placeholder').html('<input type="text" name="discount[]" class="discount_service_input" onfocus="formatDiscountOnFocus(this)" onblur="formatDiscountServiceOnBlur(this)">');
        existingRow.find('km_placeholder').html('<input type="number" name="km[]" class="km_input" onchange="updateServicePrice(this)"></td>');
...
function addAccessoryRow(data) {
    ...
    const existingRow = tableBody.find(`tr[data-accessory-id="${data.accessories_id}"]`);

    if (existingRow.length > 0) {
        if (existingRow.find('.accessories_id_input').length === 0) {
            existingRow.append(`<td style="display:none;"><input type="hidden" class="accessories_id_input" value="${data.accessories_id || ''}"></td>`);
        }
        return;
    }
    const newRow = $('<tr>').attr('data-quantity-id', data.accessories_id || '');
    newRow.append(`<td style="display:none;"><input type="hidden" class="accessories_id_input" value="${data.accessories_id || ''}"></td>`)
    newRow.append(`<td class="accessories_description_placeholder">${data.accessories_description || ''}</td>`);
    newRow.append(`<td><input type="text" class="accessories_quantity_input" value="${data.accessories_quantity || ''}"></td>`);
    newRow.append(`<td><input type="number" class="meters_input" value="${data.meters || ''}"></td>`);
    newRow.append(`<td><input type="text" class="accessories_unit_price_input" value="${data.accessories_unit_price || ''}"></td>`);
    newRow.append(`<td><input type="text" class="accessories_days_input" value="${data.accessories_days}"></td>`);
    newRow.append(`<td><input type="text" class="items_meters_input" value="${data.items_meters}"></td>`);
    newRow.append(`<td><input type="text" class="accessories_discount_input" value="${data.accessories_discount || '0%'}"></td>`);
    const price = data.accessories_price || 'R$ 0,00';
    newRow.append(`<td class="accessories_price_placeholder">${price}</td>`);

    const deleteButton = $('<button class="codigos_excluir" onclick="removeAccessory(this.parentNode.parentNode)">Excluir</button>');
    newRow.append($('<td>').append(deleteButton));
...
function updatePricesForAllAccessories() {
...
function submitProposal() {
    ...
    .then(data => {
        if (data.success) {
            alert("Proposta atualizada e salva com sucesso!");
...
function collectAccessories() {
    ...
    $('#accessories_table_body tr').each(function () {
        ...
        const description = row.find('.accessories_description_placeholder').text().trim();
        const quantity = row.find('.accessories_quantity_input').val()?.trim() || '';
        const unit_price = row.find('.accessories_unit_price_input').val()?.trim() || '';
        const discount = row.find('.accessories_discount_input').val()?.trim() || '';
        const days = row.find('.accessories_days_input').val()?.trim() || '';
        const price = row.find('.accessories_price_placeholder').text()?.trim() || '';
        ...
        if (isManual) {
            accessory.meters = 0;
            accessory.items_meters = 0;
        } else {
            accessory.meters = row.find('.meters_input').val()?.trim() || 0;
            accessory.items_meters = row.find('.items_meters_input').val()?.trim() || 0;
        }

        const accessories_id = row.find('.accessories_id_input').val()?.trim();
        if (accessories_id !== '') {
            accessory.accessories_id = accessories_id;
        }
